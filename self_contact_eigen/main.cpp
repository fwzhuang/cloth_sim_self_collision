...
#include "../utility.h"              // 向量，四元数运算
#include "../solve_internal_eigen.h" // 使用Eigen库的布料求解器
#include "../self_collision_cloth.h" // 解决自冲突的库

/* ------------------------------------------------------------------------ */

// Setting problem here
void SetClothShape_Square(std::vector<double> &aXYZ0, // (out) undeformed vertex positions，转换前的顶点位置数组
                          std::vector<int> &aBCFlag,  // (out) boundary condition flag (0:free 1:fixed)，边界条件标志
                          std::vector<int> &aTri,     // (out) index of triangles，三角形顶点索引
                          std::vector<int> &aQuad,    // (out) index of 4 vertices required for bending，用于计算Bending的4个顶点的数组
                          double &total_area,         // (out) total area of cloth，布面积
                          ///
                          double elem_length, // (in) number of division of the square cloth edge, 每边的分割数
                          double cloth_size_x,
                          double cloth_size_z) // (in) size of square cloth，一侧长度
{
  // make vertex potision array 创建顶点位置数组
  ...
  // make triangle index array, 创建三角形的顶点数组
...
const double lambda = 0.0;                // Lame's（拉米） 1st parameter(弹性模量)，拉米第一定数
const double myu = 30.0;                  // Lame's 2nd parameter（剪切模量），拉米第二定数
const double stiff_bend = 1.0e-2;         // bending stiffness.
const double areal_density = 0.1;         // areal density of a cloth
const double gravity[3] = {0, 0.05, -10}; // gravitatinal accereration.
double time_step_size = 0.015;            // size of time step.
...
// 自接触功能
int iroot_bvh;                  // BVH树结构的根节点的索引
std::vector<CNodeBVH> aNodeBVH; // BVH节点阵列
std::vector<CAABB3D> aBB_BVH;   // AABB数组，大小是节点数
CJaggedArray aEdge;

std::vector<double> aNormal; // deformed vertex noamals，(可視化用)
...
{ // make normal
  ...
  for (int itri = 0; itri < ntri; itri++)
  {
    ...
    double n[3], area;
    UnitNormalAreaTri3D(n, area, c[0], c[1], c[2]);
    aNormal[ip0 * 3 + 0] += n[0];
    aNormal[ip0 * 3 + 1] += n[1];
    aNormal[ip0 * 3 + 2] += n[2];
    aNormal[ip1 * 3 + 0] += n[0];
  ...
  {
    double sqlen =
        +aNormal[ip * 3 + 0] * aNormal[ip * 3 + 0] + aNormal[ip * 3 + 1] * aNormal[ip * 3 + 1] + aNormal[ip * 3 + 2] * aNormal[ip * 3 + 2];
