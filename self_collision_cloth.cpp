...
// 撃力を計算
void SelfCollisionImpulse_Proximity(std::vector<double> &aUVWm, // (in,out)velocity
									////
									double delta,
									double stiffness,
									double dt,
...
// Impulseの計算
void SelfCollisionImpulse_CCD(std::vector<double> &aUVWm, // (in,out)velocity
							  ////
							  double delta,
							  double stiffness,
							  double dt,
...
// RIZを更新する
void MakeRigidImpactZone(std::vector<std::set<int>> &aRIZ,				   // (in,ou)RIZに属する節点のインデックスの集合の配列
						 ...
						 const CJaggedArray &aEdge)						   // 三角形メッシュの辺の配列
{
	for (int ice = 0; ice < aContactElem.size(); ice++)
	{
		...
		{									  // overlapping two reagion，接触要素が複数のRIZに接するー＞接する複数のRIZをマージする
			std::vector<std::set<int>> aRIZ1; // マージされた後のRIZの配列
			for (int iriz = 0; iriz < aRIZ.size(); iriz++)
			{ // 接さないRIZをコピー
				if (ind_inc.find(iriz) != ind_inc.end())
					continue;
			...
			aRIZ1[ind0].insert(n[0]);
			aRIZ1[ind0].insert(n[1]);
			aRIZ1[ind0].insert(n[2]);
			aRIZ1[ind0].insert(n[3]);
			aRIZ = aRIZ1;
		}
	}
}
...
{
	const double det =
		+a[0] * a[4] * a[8] + a[3] * a[7] * a[2] + a[6] * a[1] * a[5] - a[0] * a[7] * a[5] - a[6] * a[4] * a[2] - a[3] * a[1] * a[8];
...
void ApplyRigidImpactZone(std::vector<double> &aUVWm, // (in,out)RIZで更新された中間速度
						  ////
...
// 返回解决碰撞的中间速度
void GetIntermidiateVelocityContactResolved(std::vector<double> &aUVWm,
