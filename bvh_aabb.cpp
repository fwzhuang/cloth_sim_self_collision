...
static inline void AddPoint(CAABB3D &bb, const CVector3D &p, double eps)
{
    bb.AddPoint(p.x, p.y, p.z, eps);
}

// 建立BVH边界框，用于三角形的接近度计算
void BuildBoundingBoxChild_Prx(int ibvh,
                               double delta,
...
void BuildBoundingBoxChild_CCD(int ibvh,
                               double dt,
                               const std::vector<double> &aXYZ,
...
// 构造一个包围三角形的三角形
static void MakeTriSurTri(std::vector<int> &aTriSur,
                          const int nnode/*v_size/3*/,
                          const std::vector<int> &aTri)
{
    ...
    CJaggedArray aTSV;
    aTSV.SetNodeToElem(aTri, nTri, 3, nnode);
...
CVector3D TriGravityCenter(int itri,
                           const std::vector<int> &aTri,
                           const std::vector<double> &aXYZ)
{
    int ino0 = aTri[itri * 3 + 0];
...
void DevideTriAryConnex(int iroot_node,
                        ...
                        const std::vector<int> &aTriSur)
{
    ...
    for (int il = 0; il < list.size(); il++)
    {
        ...
        if (fabs(Dot(gc0 - org, dir)) < 1.0e-10)
            continue;
        if (Dot(gc0 - org, dir) < 0)
        {
            dir *= -1;
        }
        itri_ker = itri0;
...
int MakeBVHTopology_TopDown(const std::vector<int> &aTri,
                            const std::vector<double> &aXYZ,
                            std::vector<CNodeBVH> &aNodeBVH)
{
...
// VFの距離
double DistanceFaceVertex(const CVector3D &p0, const CVector3D &p1, const CVector3D &p2,
                          const CVector3D &p3,
                          double &w0, double &w1)
{
    CVector3D v20 = p0 - p2;
...
bool IsContact_FV_Proximity(int ino0, int ino1, int ino2, int ino3,
                            ...
                            const double delta)
{
    if (ino3 == ino0 || ino3 == ino1 || ino3 == ino2)
    {
        return false;
    }
    if (!bb.IsInside(p3.x, p3.y, p3.z))
        return false;
...
bool IsContact_EE_Proximity(int ino0, int ino1, int jno0, int jno1,
                            ...
                            const double delta)
{
    if (ino0 == jno0 || ino0 == jno1 || ino1 == jno0 || ino1 == jno1)
        return false;
    if (q0.x + delta < p0.x && q0.x + delta < p1.x && q1.x + delta < p0.x && q1.x + delta < p1.x)
        return false;
    if (q0.x - delta > p0.x && q0.x - delta > p1.x && q1.x - delta > p0.x && q1.x - delta > p1.x)
        return false;
    if (q0.y + delta < p0.y && q0.y + delta < p1.y && q1.y + delta < p0.y && q1.y + delta < p1.y)
        return false;
    if (q0.y - delta > p0.y && q0.y - delta > p1.y && q1.y - delta > p0.y && q1.y - delta > p1.y)
        return false;
    if (q0.z + delta < p0.z && q0.z + delta < p1.z && q1.z + delta < p0.z && q1.z + delta < p1.z)
        return false;
    if (q0.z - delta > p0.z && q0.z - delta > p1.z && q1.z - delta > p0.z && q1.z - delta > p1.z)
        return false;
                                 ...
                                 const std::vector<CAABB3D> &aBB)
{
    ...
    if (!aBB[ibvh0].IsIntersect(aBB[ibvh1]))
        return;
...
// 三次関数を評価する関数
inline double EvaluateCubic(double r2,                                  // 入力の値
...
static void BisectRangeCubicRoot(int &icnt,                                  // (in out)何回幅を狭めたかというカウンタ
...
// 三次関数の根を探す関数
static double FindRootCubic(double r0, double r1,
                            double v0, double v1,
...
double FindCoplanerInterp(const CVector3D &s0, const CVector3D &s1, const CVector3D &s2, const CVector3D &s3,
                          const CVector3D &e0, const CVector3D &e1, const CVector3D &e2, const CVector3D &e3)
{
    ...
    if (fabs(k3) < 1.0e-10 && fabs(k2) > 1.0e-10)
    {                               // quadric function、二次関数
                      ...
                      const CAABB3D &bb)
{
    ...
    double dist = DistanceFaceVertex(p0, p1, p2, p3, r0, r1);
    {
        ...
        if (r0 < 0 || r0 > 1 || r1 < 0 || r1 > 1 || r2 < 0 || r2 > 1)
        {
            ...
            if (dist01 > max_app && dist12 > max_app && dist20 > max_app)
            {
                return false;
            }
        }
